...
export function OrchestrationConsole({ productName, productDescription, activeRole }: OrchestrationConsoleProps) {
  ...
  const handleGateAction = async (action: 'approve' | 'modify' | 'rerun' | 'pause') => {
    ...
    addLog('[PRD Creator] Sending prompt to the Gemini backend (/api/evaluate)...');
    ...
    try {
      ...
      // Sandbox output is templated text written in advance, not model output.
      // Say so in the result and the log instead of presenting it as a run.
      if (data.sandbox) addLog('[PRD Creator] No GEMINI_API_KEY: showing sandbox template output, not a model run.');
      setEvaluationResult(
        data.sandbox
          ? `> **Sandbox output.** No model was called; this is template text for demo purposes.\n\n${data.text}`
          : data.text
      );
